...
//
// withdraw rejects when invoked by anyone other than the authorized signer
//
...
const PROGRAM_ID: &str = "Ad111tRef1111111111111111111111111111111111";
/// Compiled target program. For a deployed program, produce this
/// with: `solana program dump Ad111tRef1111111111111111111111111111111111 <name>.so`.
const PROGRAM_SO: &str = "../../target/deploy/admin_ref.so";
...
impl AdminrefAccessFixture {
    ...
    /// Attacker arm — probes `withdraw` with a freshly-minted Keypair
    ...
    pub fn action_attack_withdraw(&mut self, #[range(1..1_000_000)] amount: u64) -> bool {
...
//
// If the program ever accepted a `withdraw` call signed by an
// attacker instead of the legitimate signer(s), the sticky flag is
// `true` and this assertion fails.
#[invariant_test]
fn invariant_withdraw_rejects_unauthorized(fixture: &mut AdminrefAccessFixture) {
    fuzz_assert_eq!(
        fixture.unauthorized_success_observed, false,
        "unauthorized withdraw succeeded against program {}",
        fixture.program_id
    );
}
